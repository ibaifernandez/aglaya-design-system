...
5. `_read()` - 10 edges
6. `_signature_terms()` - 9 edges
7. `_product()` - 9 edges
8. `AGLAYA Design System` - 9 edges
...
## Knowledge Gaps
- **93 isolated node(s):** `aglaya-ds-mcp/.venv/bin/python`, `aglaya-ds-mcp`, `EXCLUSIONS`, `FOOTER_LINKS`, `PROBLEMS` (+88 more)
...
- **What connects `aglaya-ds-mcp/.venv/bin/python`, `aglaya-ds-mcp`, `EXCLUSIONS` to the rest of the system?**
