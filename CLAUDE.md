...
1. [`README.md`](README.md) — la portada pública, en inglés: el sistema visual (tokens, tipografía, iconografía) y cómo se usa.
2. [`docs/BRAND-RULES.md`](docs/BRAND-RULES.md) — **el canon de voz y los no-negociables**, que el MCP sirve leyéndolos de ahí en vivo. Está en castellano a propósito y sus títulos son interfaz: el MCP los busca por texto exacto.
...
**Y los punteros los vigila el segundo guardián.** [`tools/guard_punteros.py`](tools/guard_punteros.py) recorre **los ficheros de texto versionados**, no solo los `.md` — se llamaba «de punteros» y solo miraba markdown, y por eso dio verde sobre el PR que borró un fichero dejando tres referencias vivas en `LICENSE`, en el CSS canónico y en un `pyproject.toml`. Falla por tres motivos: un **enlace markdown a un archivo propio que no está donde dice** —la avería de reorganizar el repo y dejar el puntero atrás—, una **ruta rota** escrita fuera de markdown, y **una ruta interna del atlas del capitán**, que caduca en cuanto él reorganiza. Quedan fuera `graphify-out/` (regenerable), `/vendor/`, `tools/` —cuyas baterías nombran rutas rotas a propósito— y `package.json`, cuyas rutas vigila el guardián del paquete. **Reconoce el enlace y la ruta, no el nombre de fichero suelto**, y el porqué está medido en su docstring: la grafía de una retractación legítima y la de un puntero roto son idénticas, así que perseguir el nombre suelto pondría en rojo las dos. Aquí los acentos graves **no eximen**, al revés que en el guardián de la huella: una ruta se escribe casi siempre entrecomillada, así que eximirlas lo dejaría sin nada que morder. Su batería es [`tools/test_guard_punteros.sh`](tools/test_guard_punteros.sh), que además comprueba lo legítimo (enlaces externos, anclas, globs) para que no dé rojos falsos, y que no encontrar docs salga rojo en vez de dar un verde vacío.

**Y los valores los vigila el tercero.** [`tools/guard_valores.py`](tools/guard_valores.py) lee los tokens de `colors_and_type.css` **en vivo** —los de `:root` y los de los modos de color— y falla si uno de esos valores aparece copiado a mano en cualquier otro archivo versionado. Cubre el color en todas sus formas (hexadecimal, con alfa, en forma corta, como `rgb()`/`rgba()`, y el alfa escrito de cualquiera de sus grafías equivalentes) y las familias no cromáticas cuyo literal es **auto-identificable**: tracking, tipografías —por el nombre de familia, no por la pila, porque las copias cambian el fallback—, duraciones, curvas de easing, breakpoints y radios. **Lo que NO cubre está declarado token a token en su docstring, y hay que leerlo antes de darlo por vigilado**: las familias en `rem` quedan fuera a propósito, porque el mismo literal es legítimamente un espaciado y un tamaño de letra y separarlos exigiría un cuarto parser de CSS; y los valores que no identifican nada —un cero, un `none`, un compuesto de otros tokens— tampoco entran. Además cruza `products/products.json` contra los tokens del CSS, porque ese manifiesto es un segundo domicilio legítimo y desviarse en silencio sería mentir con cara de canon. **No lleva ni un valor dentro**: cambia el rojo y persigue el nuevo; añade un acento de producto y queda protegido sin tocarlo. No vigila el negro ni el blanco puros (genéricos, darían rojos falsos) ni los SVG de `assets/` y `products/`, que llevan el color pintado dentro: eso es el artefacto, no una cita, y si el color cambia hay que reexportarlos a mano. Los acentos graves **no eximen**, como en el de punteros. Su batería es [`tools/test_guard_valores.sh`](tools/test_guard_valores.sh).

**Y al paquete lo vigila el guardián que no puede ver lo que protege.** [`tools/guard_paquete.py`](tools/guard_paquete.py) lee `package.json` y falla si el paquete deja de ser instalable **desde fuera de esta máquina**: una dependencia atada a este disco (`file:`, `link:`, `workspace:`, una ruta relativa), un `repository.url` en SSH —un CI ajeno no tiene llaves—, algo declarado en `exports`/`files`/`bin` que no se entrega, o `dist/` versionado, que sería abrir una segunda casa para cada valor de marca. Además exige dos cosas de honestidad: que el CSS exportado sea el canónico y no una copia con otro nombre, y que **ninguna tipografía se redistribuya sin su licencia al lado** — empareja cada archivo de fuente con su `LICENSE-<Familia>.txt`, así que también protege a la familia que aún no existe. Esa avería ya ocurrió: el repo se hizo público y el paquete llevaba las tres familias sin su licencia. Es el guardián más fácil de dar por bueno sin serlo, porque la avería que persigue es invisible aquí: un `file:` a un repo hermano instala perfectamente en este disco y solo se rompe en el build del consumidor. Su batería es [`tools/test_guard_paquete.sh`](tools/test_guard_paquete.sh).
